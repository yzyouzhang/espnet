...
if ! "${skip_train}"; then
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asvspoof_train \
                ...
                --train_data_path_and_name_and_type "${_asvspoof_train_dir}/text,label,csv_int" \
                --valid_data_path_and_name_and_type "${_asvspoof_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asvspoof_valid_dir}/text,label,csv_int" \
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        _opts+="--train_data_path_and_name_and_type ${_asvspoof_train_dir}/text,label,csv_int "
        ...
        _opts+="--valid_data_path_and_name_and_type ${_asvspoof_valid_dir}/text,label,csv_int "

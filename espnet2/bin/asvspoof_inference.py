...
class SpeechAntiSpoof:
    ...
    def __call__(
        ...
        # b. Forward Encoder
        enc, _ = self.asvspoof_model.encode(**batch)
        ...
        score = self.asvspoof_model.decoder(enc, None)
        result = int(score > 0)
        assert check_return_type((result, float(score)))
...
def inference(
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            try:
                result, score = speech_anti_spoof(**batch)
            ...
            result_writer["antispoof"][key] = str(result)
            result_writer["score"][key] = str(score)

            logging.info("processed {}: score {}, predict {}".format(key, score, result))

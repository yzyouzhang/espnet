...
lmtag=elan_p_272_up_466_reserve          # tag for managing LMs
...
expname=train_elan_p_272_up_466_reserve_sp_pytorch_${tag}
expdir=exp/${expname}
output_dir=elan
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    python demo/scripts/data_prep_for_botany_mono_blank.py -a ${annotation_dir} -t data/${recog_set} -s ${sound_dir} -i ${recog_files} --lang nahuatl
fi
